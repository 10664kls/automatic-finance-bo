...
const DialogSelfEmployedIncomeTransaction = ({
  open,
  onClose,
  calculationNumber,
  currency,
    ...
    async (): Promise<IncomeTransaction | null> => {
      try {
        const response = await API.get<{ transaction: IncomeTransaction }>(
          `/v1/selfemployed/calculations/${calculationNumber}/transactions/${billNumber}`
        );
  ...
  return (
    <>
      <Dialog open={open} onClose={onClose} fullWidth maxWidth="lg">
        <DialogTitle>Income Transactions Preview</DialogTitle>
...
export default DialogSelfEmployedIncomeTransaction;
