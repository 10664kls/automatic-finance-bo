...
import { ListSelfEmployedWordings } from "../api/model";
import { useQuery } from "@tanstack/react-query";
import { useState } from "react";
import API from "../api/axios";
import { DateTime } from "luxon";
import DrawerAddSelfEmployedWording from "./DrawerAddSelfEmployedWording";
import DrawerEditSelfEmployedWording from "./DrawerEditSelfEmployedWording";

const TabSelfEmployedWording = () => {
  ...
  } = useQuery<ListSelfEmployedWordings>({
    queryKey: ["listSelfEmployedWordings", pageToken, pageSize],
    queryFn: async () => {
      const apiURL = new URL(
        `${import.meta.env.VITE_API_BASE_URL}/v1/selfemployed/wordlists`
      );
      ...
      const response = await API.get<ListSelfEmployedWordings>(
        apiURL.toString()
      );
  ...
  const createWording = async (payload: { word: string }): Promise<void> => {
    try {
      const response = await API.post("/v1/selfemployed/wordlists", payload);
  ...
  const updateWording = async (payload: { word: string }): Promise<void> => {
    try {
      const response = await API.put(
        `/v1/selfemployed/wordlists/${wordingID}`,
        payload
      );
      ...
      <Paper
        ...
        <Stack
          ...
          <Typography variant="h6">
            Wordings used for income from self-employment
            <Button
        ...
        <TablePagination
          ...
          slotProps={{
            actions: {
              ...
              nextButton: {
                disabled:
                  wordings && wordings.nextPageToken.length > 0 ? false : true,
                onClick: () => {
...
export default TabSelfEmployedWording;
